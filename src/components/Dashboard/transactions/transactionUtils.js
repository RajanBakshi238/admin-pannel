...
export const print = (data) => {
  ...
  pdf.html(string, {
    ...
    callback: function (doc) {
      let fileName = `${data.fullName}_${data.userName}_${data.type}.pdf`;
      doc.save(fileName);
    },
    x: 10,
    y: 20,
...
export const fetchTransactionsData = async () => {
  try {
    ...
    for (const transaction of transactionSnapshot.docs) {
      // const studentSnap = await getDoc(transaction.data().studentRef)
      // const userSnap = await getDoc(transaction.data().userRef)

      const [studentSnap, userSnap] = await Promise.all([
        getDoc(transaction.data().studentRef),
        getDoc(transaction.data().userRef),
      ]);
